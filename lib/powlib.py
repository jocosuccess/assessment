...
from conf.config import myapp, server_settings
...
class powDecNew():
    ...
    def has_many(self, child_as_str, backref=False):
        ...
        def decorator(parent_class):
            ...
            import sys
            if "assessment.models.sql." + child_module_name in sys.modules.keys():
                #print(dir(sys.modules["assessment.models.sql." + child_module_name]))
                child_klass = getattr(sys.modules["assessment.models.sql." + child_module_name], child_class_name)
            else:
                import importlib
                mod = importlib.import_module('assessment.models.sql.' + child_module_name)
                #mod = __import__('assessment.models.sql.'+rel_module_name, fromlist=[rel_class_name])
    ...
    def belongs_to(self, parent_as_str):
        ...
        def decorator(child_class):
            ...
            parent_class = None
            import sys
            if ("assessment.models.sql."+parent_module_name) in sys.modules.keys():
                parent_class = getattr(sys.modules["assessment.models.sql."+parent_module_name], parent_class_name)
            else:
                import importlib
                mod = importlib.import_module('assessment.models.sql.' + parent_module_name)
                #mod = __import__('assessment.models.sql.'+rel_module_name, fromlist=[rel_class_name])
    ...
    def one_to_one(self, child_as_str):
        ...
        def decorator(parent):
            ...
            mod = __import__('assessment.models.sql.'+child_module_name, fromlist=[child_class_name])
    ...
    def many_to_many(self, children):
        ...
        # rel_as_str is the plueral name of the child class (adresses, comments)
        # klass below is the real class instance of the child
        from database.sqldblib import Base
        def decorator(parent_class):
            parent_name = parent_class.__name__.lower()
            parent_class_name = parent_class.__name__
